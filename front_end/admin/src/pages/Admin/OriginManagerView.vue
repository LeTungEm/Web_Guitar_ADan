<template>
  <div class="overflow-y-auto max-h-[85vh]">
    <div class="text-3xl font-bold text-center">Quản Lý Xuất Xứ</div>
    <div class="flex gap-5 flex-wrap shadow-lg p-5 mb-5">
      <div
      ...
      >
        <div class="flex justify-end font-bold cursor-pointer">
          <svg
            :onclick="() => changeUpdateId(origin.key)"
        ...
        <div>{{ origin.name }}</div>
        <div v-if="updateId == origin.key" class="flex">
          <input
          ...
          <button
            ...
            class="bg-green-700 hover:bg-green-800 cursor-pointer py-1 lg:py-2 px-2 lg:px-4 text-white"
          >
            Sửa
          </button>
        </div>
    ...
    <button
      :onclick="insertOrigin"
      class="border rounded-md w-full hover:bg-green-700 hover:text-white duration-300 py-1 lg:py-2 px-2 lg:px-4"
    >
      Thêm
    </button>
  ...
  methods: {
    ...
    updateOrigin() {
      if (this.updateName.trim() != "") {
        OriginService.updateOrigin(this.updateName, this.updateId).then(() => {
          this.updateName = "";
          this.updateId = "";
          this.getOrigin();
        });
      } else {
        this.showNotification(["Hãy nhập tên mới của quốc gia!", false]);
      }
    },
    insertOrigin() {
      if (this.name.trim() != "") {
        OriginService.insertOrigin(this.name).then(() => {
          this.name = "";
          this.getOrigin();
        });
      } else {
        this.showNotification(["Hãy nhập tên quốc gia!", false]);
      }
    },
  },
};
