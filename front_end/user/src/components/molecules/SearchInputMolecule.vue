<template>
  <div class="w-full lg:w-1/2 relative mx-5 lg:mx-0">
    <div v-on:click="closeTabMobie" class="flex justify-end lg:hidden">
      <span
        class="border border-pink-500 text-pink-500 mr-5 mb-5 cursor-pointer px-3 py-1 rounded-full font-bold"
        >X</span
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    toDetailPage(cat) {
      this.$router.push(`/chi-tiet/${cat.slug}`);
      this.tabStatus = false;
      this.$emit("closeTabMobie");
    },
  },
