...
class CatService {
    ...
    getBestSeller() {
        let formData = new FormData();
        formData.append('action', 'getBestSeller');
        return axios.post(`${PRODUCT_API_URL}`, formData);
    }
    getNewProduct() {
        let formData = new FormData();
        formData.append('action', 'getNewProduct');
        return axios.post(`${PRODUCT_API_URL}`, formData);
    }
    getAllCatByCategorySlug(category_slug) {
        let formData = new FormData();
        formData.append('action', 'getAllCatByCategorySlug');
        formData.append('category_slug', category_slug);
        return axios.post(`${PRODUCT_API_URL}`, formData);
    }
    getAllParentChildByCategorySlug(category_slug) {
        let formData = new FormData();
        formData.append('action', 'getAllParentChildByCategorySlug');
        formData.append('category_slug', category_slug);
        return axios.post(`${PRODUCT_API_URL}`, formData);
    }
    searchCat(search) {
